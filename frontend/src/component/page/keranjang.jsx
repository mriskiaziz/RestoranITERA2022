...
import {
  incrementCart,
  decrementCart,
} from "../../features/keranjang/cartSlice";
...
class Keranjang extends React.Component {
  ...
  async sendOrder() {
    try {
      await axios
        .post(process.env.REACT_APP_BACKEND_URL + "/order", {
          customer: this.state.customer,
          table: this.state.table,
          value: localStorage.getItem("cart")
            ? JSON.parse(localStorage.getItem("cart")).value
            : {},
        })
        .then((res) => {
          localStorage.clear();
  ...
  componentDidUpdate() {
    if (document.getElementById("update_total") !== null) {
      document.getElementById("update_total").innerHTML = uangRupiah(
        this.state.total
      );
    }
  }
          ...
          <Container className="mt-5">
            ...
            {this.state.dataCart != null && this.state.dataMenu != null ? (
              this.state.dataCart.map((e, i) => {
                return (
                  <Row
                    className="border border-1 shadow-sm mx-1 my-3 p-3 mb-3"
                    key={i}
                  >
                      ...
                      <Button variant="outline-primary">
                        <AiOutlinePlus
                          ...
                          onClick={() => {
                            ...
                            var amount =
                              this.state.total +
                              this.state.dataMenu[e.uid].harga;
                            this.setState({
                              dataCart: JSON.parse(localStorage.getItem("cart"))
                                .value,
                              total: amount,
                ...
                <Row className="d-flex flex-column-reverse flex-md-row ">
                  <Col className="col-6 col-md-3 px-2 py-1 d-flex justify-md-content-center ">
                    <Form.Select
                      ...
                      onChange={(data) =>
                        this.setState({ table: data.target.value })
                      }
                    >
